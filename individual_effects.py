...
AUGMENTATIONS = {
    ...
    "median_blur": [
        v2.CenterCrop(224),
        v2.RandomApply([monai_transforms.MedianSmooth(radius=3)], p=0.5),
    ],
    "grid_distortion": [
        v2.CenterCrop(224),
        monai_transforms.RandGridDistortion(prob=1.0),
    ],
    "gaussian_noise": [
        v2.CenterCrop(224),
...
def objective(trial, args):
    ...
    augmentation_name = trial.suggest_categorical("transform", AUGMENTATIONS.keys())
    augmentation = AUGMENTATIONS[augmentation_name]

    # Define the train and val transforms
    train_transform = v2.Compose(
        [v2.ToTensor(), v2.Resize(224, antialias=True)]
        + augmentation
...
def main():
    ...
    )  # Use n_trials=len(AUGMENTATIONS.keys()) for full study
