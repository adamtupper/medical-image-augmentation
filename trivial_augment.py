...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="The random seed to use to initialize model training",
    )
    parser.add_argument(
        "--dataset", type=str, default="busi", help="The dataset to use"
    )
    parser.add_argument(
        "--subset", type=str, default="all", help="The subset of augmentations to use"
    )

    args = parser.parse_args()
    args.log_dir = os.path.join(args.log_dir, args.dataset, args.subset)
...
def get_augmentation_set(dataset, subset):
    if subset == "all":
        return ALL
        ...
        if subset == "top5":
            ...
            return BUS_BRA_TOP10
    elif dataset == "bus_bra_birads":
        if subset == "top5":
            return BUS_BRA_BIRADS_TOP5
        elif subset == "top10":
            return BUS_BRA_BIRADS_TOP10
